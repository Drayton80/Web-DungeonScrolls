...
function generate_experience_calculator_formulary(select_id) {
    ...
    var div_object_experience_received = document.getElementById("experience_calculator_result");
    var select = document.getElementById(select_id);

    system_selected_id = select.options[select.selectedIndex].value;

    if (system_selected_id != null && system_selected_id != "") {
        div_object_experience_received.innerHTML = ""
        ...
        $.ajax({
            ...
            success: function (response_data) {
                ...
                html_text += '<button type="button" class="btn btn-outline-dark btn-block" onclick="calculate_experience(' + system_selected_id + ')">Calculate</button>'
                html_text += '</div>'
                html_text += '</div>'
...
function calculate_experience(system_selected_id) {
    ...
    number_of_characters = Number(document.getElementById('number_of_characters').value);

    if (!isNaN(number_of_characters) && number_of_characters > 0) {
        for (index in all_difficulty_levels) {
        ...
        $.ajax({
            ...
            success: function (response_data) {
                ...
                } else if (system_selected_id == 3) {
                    h2_title = '<h2 class="mb-4"> Experience Received </h2>';

                    for (character_level in response_data.experience_points_per_level) {
                        character_experience_received = response_data.experience_points_per_level[character_level]

                        if (character_experience_received != "0") {
                            xp_result_text += '<div class="col p-0">'
                            xp_result_text += '<button type="button" class="btn btn-outline-dark btn-block" disabled>'
                            xp_result_text += 'Every single Character engaged in battle receive ' + character_experience_received + ' XP'
                            xp_result_text += '</button>'
                            xp_result_text += '</div>'
                        }
